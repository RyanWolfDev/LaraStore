...
const discord = require('discord.js')
    ...
    if(!findU) return message.channel.send(`Desculpe, não te encontrei no meu banco de dados.`)
    ...
    if(!find1L) return message.channel.send(embed.setDescription(`Aparentemente você não é nosso cliente!`))
    ...
    message.author.send(embed.addField('Plugin | Licença',mapa.join('\n'))).catch(e => {
        if(e) return message.channel.send(`Não consegui enviar seus dados no privado!`)
    })
    //console.log(findL)
